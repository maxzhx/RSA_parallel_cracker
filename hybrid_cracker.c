...
void mpi_primeFactors(mpz_t n) {
  ...
  #pragma omp parallel
  {
    ...
    #pragma omp master
      printf("... running with %d threads per node...\n", threads);
    int count = 0;
    mpz_t i;
    mpz_init(i);
    mpz_t t;
    ...
    for (mpz_sub_ui(i, sq, (my_rank-1)*2+2*(num_procs-1)*thread_num); mpz_cmp_ui(i, 3) >= 0 && !flag1; mpz_sub_ui(i, i, 2*(num_procs-1)*threads)) {
      // gmp_printf("node %d thread %d : %Zd\n", my_rank, thread_num, i);
      // check if other process succeed, every 1000000 time, to increase efficiency
      // #pragma omp critical
      if (thread_num == 0 && count++ % 5000000 == 0) {
      ...
      if (mpz_cmp_ui(t, 0) == 0) {
        flag1 = 1;
        gmp_printf("... prime factor %Zd found by node %d ...\n", i, my_rank);
