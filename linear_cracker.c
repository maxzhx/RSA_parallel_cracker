...
// void primeFactors_primeonly(mpz_t result, mpz_t n) {
...
//     mpz_add(sq, sq, n);
//     mpz_sqrt(sq, sq);
//     for (mpz_set_ui(i, 3); mpz_cmp(i, sq) <= 0; mpz_nextprime(i,i)) {
//         mpz_mod(b, n, i);
//         if ( mpz_cmpabs_ui(b, 0) == 0) {
//           mpz_set(result, i);
//           break;
...
void primeFactors(mpz_t result, mpz_t n) {
  // Print the number of 2s that divide n
  mpz_t b;
  mpz_t sq;
  mpz_init(b);
  ...
  mpz_add(sq, sq, n);
  mpz_sqrt(sq, sq);
  // turn sq to an odd number
  mpz_mod_ui(b, sq, 2);
  mpz_sub_ui(sq, sq, !mpz_cmpabs_ui(b, 0));
  gmp_printf("n: %Zd sq: %Zd\n", n, sq);

  // n must be odd at this point.  So we can skip one element (Note i = i +2)
  for (; mpz_cmpabs_ui(sq, 3) >= 0; mpz_sub_ui(sq, sq, 2)) {
